document.addEventListener('DOMContentLoaded', () => {
    iniciarSimuladorMegaSena();
});

function iniciarSimuladorMegaSena() {
    // Estado (traduzido)
    ...
    function adicionarAoHistorico(resultados) {
        const historico = JSON.parse(localStorage.getItem('mega_history') || '[]');
        const novoItem = {
        ...
        localStorage.setItem('mega_history', JSON.stringify(historico));
    ...
    // Acessível globalmente para os botões do HTML
            ...
            if (item.games) {
                item.games.forEach(g => {
                    ...
                    const labelAcertos = acertos > 0 ? (
                        acertos === 4 ? `<span class="win-badge badge-quadra">QUADRA</span>` :
                            acertos === 5 ? `<span class="win-badge badge-quina">QUINA</span>` :
                                acertos === 6 ? `<span class="win-badge badge-sena">MEGA SENA</span>` :
    ...
    async function buscarConcurso(concurso) {
        ...
        const chaveCache = `megasena_result_${concurso}`;
        ...
        try {
            ...
            const guidiUrl = `https://api.guidi.dev/loteria/megasena/${isLatest ? 'ultimo' : concurso}`;
            ...
            if (!dadosAPI) {
                const herokuUrl = `https://loteriascaixa-api.herokuapp.com/api/megasena/${isLatest ? 'latest' : concurso}`;
                let rawHeroku = await tentarFetch(herokuUrl);
                if (rawHeroku) dadosAPI = rawHeroku;
            }

            if (!dadosAPI) {
                let rawProxy = await tentarFetch(`/api/megasena-proxy/${concurso}`);
                if (rawProxy) dadosAPI = rawProxy;

                if (!dadosAPI) {
                    rawProxy = await tentarFetch(`http://localhost:3000/api/megasena-proxy/${concurso}`);
                    if (rawProxy) dadosAPI = rawProxy;
                }
            }
            ...
            localStorage.setItem(`megasena_result_${dadosAPI.numero}`, JSON.stringify(dadosAPI));
    ...
    function renderizarResultadosConcurso(dadosExtras) {
        ...
        if (ocorreuAcumulo) {
            ...
            let ganhadorSena = null;
            if (dadosExtras.premiacoes && Array.isArray(dadosExtras.premiacoes)) {
                ganhadorSena = dadosExtras.premiacoes.find(p => p.faixa === 'Sena' || p.descricao?.toLowerCase().includes('sena') || p.faixa === 1);
            }

            if (ganhadorSena) {
                premioEmDinheiro = ganhadorSena.valorPremio;
